...
| [`rhel9-poc-golden-template.yaml`](rhel9-poc-golden-template.yaml) | `rhel9-poc-golden` 커스텀 골든 이미지 기반 POC 전용 Template |
...
## rhel9-poc-golden-template 사용법

`rhel9-poc-golden` 커스텀 골든 이미지를 부팅 소스로 사용하는 POC 전용 템플릿입니다.

### 사전 조건

`00-init/pvc-to-qcow2.md` Part 2 절차에 따라 `rhel9-poc-golden` DataSource가 등록되어 있어야 합니다.
...
```bash
oc apply -f rhel9-poc-golden-template.yaml -n openshift
```
