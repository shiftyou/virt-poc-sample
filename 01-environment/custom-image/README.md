...
| 파일 | 설명 |
|------|------|
| [`upload-image.sh`](upload-image.sh) | 이미지 업로드 대화형 스크립트 (로컬/HTTP 방식 선택) |
| [`datavolume-http.yaml`](datavolume-http.yaml) | HTTP URL에서 이미지를 가져오는 DataVolume 템플릿 |
| [`datasource.yaml`](datasource.yaml) | 업로드 완료 후 UI 등록용 DataSource 템플릿 |
