...
5. 설정:
   ...
   - Installed Namespace: `poc-oadp` (새로 생성)
...
```bash
# Operator 설치 상태 확인
oc get csv -n poc-oadp

# OADP Pod 상태 확인
oc get pods -n poc-oadp

# Velero 확인
oc get deployment velero -n poc-oadp
```
...
```bash
# OADP Operator 로그 확인
oc logs -n poc-oadp deployment/openshift-adp-controller-manager

# Velero 로그 확인
oc logs -n poc-oadp deployment/velero
